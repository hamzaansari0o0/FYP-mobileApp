...
const PayoutCard = ({ booking, onApprove, isProcessing }) => {
  ...
  return (
    <View style={tw`bg-white p-4 rounded-lg shadow-md mb-4`}>
      <Text style={tw`text-base text-gray-600`}>Player: {booking.playerName}</Text>
      ...
      >
        {isProcessing ? (
          ...
          <Text style={tw`text-white text-center font-bold`}>Approve Payout</Text>
        )}
      </Pressable>
    </View>
  );
...
export default function PayoutsScreen() {
  ...
  const fetchPendingPayouts = async () => {
    ...
    try {
      ...
      if (error.code === 'failed-precondition') {
        Alert.alert('Index Required', 'Please check your terminal for the Firestore index link.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleApprovePayout = async (booking, adminCommission, ownerPayout) => {
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        // Step 2: Add 95% to owner balance
        ...
        if (!adminRevDoc.exists()) {
          ...
          transaction.update(adminRevenueRef, { totalRevenue: increment(adminCommission) });
        }
      });

      Alert.alert(
        'Payout Approved!',
        `Rs. ${ownerPayout} sent to Owner and Rs. ${adminCommission} added to Admin revenue.`
      );
      fetchPendingPayouts();
    } catch (error) {
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-gray-100`}>
      <View style={tw`p-5`}>
        <Text style={tw`text-3xl font-bold text-purple-800 mb-5`}>Pending Payouts</Text>
